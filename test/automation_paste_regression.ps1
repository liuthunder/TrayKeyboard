...
$resultPath = Join-Path $PSScriptRoot 'automation-paste-result.txt'
Remove-Item $resultPath -ErrorAction SilentlyContinue

Set-Clipboard -Value 'XYZ'

$form = New-Object System.Windows.Forms.Form
$form.Text = 'TrayKeyboard Paste Test'
$form.Width = 400
$form.Height = 240
...
$invokeTimer.Add_Tick({
    ...
    [NativeMethods]::SendMessage($trayWindow, 0x8001, [IntPtr]1005, [IntPtr]0x0201) | Out-Null
    [NativeMethods]::SendMessage($trayWindow, 0x8001, [IntPtr]1005, [IntPtr]0x0202) | Out-Null
})
...
$assertTimer.Add_Tick({
    ...
    if ($actual -eq 'XYZ' -and $focusState) {
